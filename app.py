...
class UserRequest(BaseModel):
    username: str
    answers: Dict[str, List[str]]  # Novo campo
...
def execute_scraper(username: str) -> subprocess.CompletedProcess:
    ...
    command = shlex.split(f"python scraper -t 5 -u {username}")
    try:
...
@app.post("/scrape")
async def scrape(user_request: Dict[str, Any]):
    start_time = time.time()

    # Extração de campos específicos do JSON recebido
    username = user_request.get("twitterData", {}).get("username", "")
    answers = user_request.get("answers", {})
